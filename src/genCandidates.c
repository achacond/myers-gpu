...
{      
	...
	lastEntriesPerQuery = (sizeCurrentQuery / SIZE_HW_WORD) + ((sizeCurrentQuery % SIZE_HW_WORD) ? 1 : 0);
...
int char2bin(unsigned char *query, qryEntry_t *binaryQuery, uint pos, uint numBases)
{
	int i; 
	uint indexBase;
	uint bitmapA 	= 0;
	...
	for(i = 0; i < numBases; i++){
		indexBase = base2number(query[pos + i]);
	...
	binaryQuery->bitmap[0] = bitmapA;
	binaryQuery->bitmap[1] = bitmapC;
	binaryQuery->bitmap[2] = bitmapG;
	binaryQuery->bitmap[3] = bitmapT;
	binaryQuery->bitmap[4] = bitmapN;
...
int transformQueries(void *queries)
{
	...
	if(qry->sizeQueries != 0){
		...
		qry->totalQueriesEntries = numEntriesPerQuery * qry->numQueries;
		qry->h_queries 	= (qryEntry_t *) malloc(qry->totalQueriesEntries * sizeof(qryEntry_t));
		if (qry->char_queries == NULL) return (31);

		for(idQuery = 0; idQuery < qry->numQueries; idQuery++){
			intraQuery = 0;
			for(pos = 0; pos < qry->sizeQueries; pos += 32){
				char2bin(qry->char_queries + (idQuery * qry->sizeQueries), qry->h_queries + (idQuery * numEntriesPerQuery) + intraQuery, 
					     pos, MIN(qry->sizeQueries - pos, 32));
				intraQuery++;
			}
		}

	}else{
		//TODO: MemoryAllocation for multiple sizes of queries
		//		-> Recorrer todos los tamaños e ir accumulando el numEntriesPerQuery;
	}
...
int saveQueries(const char *fn, void *query)
{
    ...
    sprintf(qryFileOut, "%s.%u.%u.%u.bin.qry", fn, qry->numQueries, qry->numCandidates / qry->numQueries, qry->sizeQueries);
    ...
    fwrite(qry->h_candidates, 	sizeof(candInfo_t), 	qry->numCandidates, 		fp);
