...
#else
inline __device__ void shared_collaborative_shift(uint32_t value_A, uint32_t value_B, uint32_t value_C, uint32_t value_D,  
					       						  ...
					       						  uint32_t* res_A, uint32_t* res_B, uint32_t* res_C, uint32_t* res_D)
{
	uint32_t carry;
		...
		value_C = __funnelshift_lc(carry_B, value_C, 1);
...
inline __device__ void shared_collaborative_sum(const uint32_t a_A, const uint32_t a_B, const uint32_t a_C, const uint32_t a_D, 
...
#ifdef BALLOT
inline __device__ void ballot_collaborative_shift(uint32_t value_A, uint32_t value_B, uint32_t value_C, uint32_t value_D, 
					       						  const uint32_t localThreadIdx, const uint32_t intraWarpIdx,
					       						  uint32_t* res_A, uint32_t* res_B, uint32_t* res_C, uint32_t* res_D)
{
	uint32_t carry;
	carry = ((__ballot(value_D >> 31) << 1) & (1 << intraWarpIdx)) != 0;
	carry = (localThreadIdx) ? carry : 0;
	value_D = (value_C >> 31) | (value_D << 1);
	value_C = (value_B >> 31) | (value_C << 1);
	value_B = (value_A >> 31) | (value_B << 1);
...
inline __device__ void ballot_collaborative_sum(const uint32_t a_A, const uint32_t a_B, const uint32_t a_C, const uint32_t a_D, 
					 							const uint32_t b_A, const uint32_t b_B, const uint32_t b_C, const uint32_t b_D,
					 							const uint32_t localThreadIdx, const uint32_t intraWarpIdx,
					 							uint32_t* sum_A, uint32_t* sum_B, uint32_t* sum_C, uint32_t* sum_D)
{
		...
		if((positionRef < sizeRef) && ((sizeRef - positionRef) > sizeCandidate)){
			...
			for(idEntry = 0; idEntry < numEntriesPerCandidate; idEntry++){
				...
				for(intraBase = 0; intraBase < BASES_PER_ENTRY; intraBase++){	
					...
					#else
						#ifdef BALLOT
							ballot_collaborative_sum(tEq_A, tEq_B, tEq_C, tEq_D, Pv_A, Pv_B, Pv_C, Pv_D, 
													 intraQueryThreadIdx, intraWarpIdx,
													 &sum_A, &sum_B, &sum_C, &sum_D);
						#else
							shared_collaborative_sum(tEq_A, tEq_B, tEq_C, tEq_D, Pv_A, Pv_B, Pv_C, Pv_D, 
					...
					#else
						#ifdef BALLOT
							ballot_collaborative_shift(Ph_A, Ph_B, Ph_C, Ph_D, 
													   intraQueryThreadIdx, intraWarpIdx, 
													   &Ph_A, &Ph_B, &Ph_C, &Ph_D);
							ballot_collaborative_shift(Mh_A, Mh_B, Mh_C, Mh_D, 
													   intraQueryThreadIdx, intraWarpIdx, 
													   &Mh_A, &Mh_B, &Mh_C, &Mh_D);
						#else
							shared_collaborative_shift(Ph_A, Ph_B, Ph_C, Ph_D, intraQueryThreadIdx, 
													   intraWarpIdx, localInterBuff, 
													   &Ph_A, &Ph_B, &Ph_C, &Ph_D);
							shared_collaborative_shift(Mh_A, Mh_B, Mh_C, Mh_D, intraQueryThreadIdx, 
													   intraWarpIdx, localInterBuff, 
													   &Mh_A, &Mh_B, &Mh_C, &Mh_D);
						#endif
					#endif
