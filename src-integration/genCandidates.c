...
int transformQueries(void *queries)
{
	...
	int intraBasesProcessed;

	if(qry->sizeQueries != 0){
		numEntriesPerQuery	= (qry->sizeQueries / NUM_BASES_ENTRY) + ((qry->sizeQueries % NUM_BASES_ENTRY) ? 1 : 0);
		...
		for(idQuery = 0; idQuery < qry->numQueries; idQuery++){
			intraQuery = 0;
			startQueryEntry = idQuery * numEntriesPerQuery;
			for(pos = 0; pos < qry->sizeQueries; pos += NUM_BASES_ENTRY){
				for(intraBMP = 0; intraBMP < NUM_SUB_ENTRIES; intraBMP++){
					intraBasesProcessed = SIZE_HW_WORD * intraBMP;
					char2bin(qry->char_queries + processedBases + pos + intraBasesProcessed, intraBMP, qry->h_queries + startQueryEntry + intraQuery, 
						     MIN((int)qry->sizeQueries - (pos + intraBasesProcessed), SIZE_HW_WORD));
				}
				intraQuery++;
			}
			processedBases += qry->sizeQueries;
		}
	}else{
		for(idQuery = 0; idQuery < qry->numQueries; idQuery++)
		...
		for(idQuery = 0; idQuery < qry->numQueries; idQuery++){
			...
			for(pos = 0; pos < qry->h_infoQueries[idQuery].size; pos += NUM_BASES_ENTRY){
				for(intraBMP = 0; intraBMP < NUM_SUB_ENTRIES; intraBMP++){
					intraBasesProcessed = SIZE_HW_WORD * intraBMP;
					char2bin(qry->char_queries + processedBases + pos + intraBasesProcessed, intraBMP, qry->h_queries + startQueryEntry + intraQuery, 
						     MIN((int)qry->sizeQueries - (pos + intraBasesProcessed), SIZE_HW_WORD));
			...
			processedBases += qry->h_infoQueries[idQuery].size;
		}
	}
