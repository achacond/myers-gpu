...
int computeMyers(qryEntry_t *h_queries, uint32_t *h_reference, resEntry_t *h_results, 
 				  ...
 				  uint32_t numEntriesPerQuery, uint32_t numLargeEntriesPerQuery, uint32_t positionRef, uint32_t initEntry, 
				  uint32_t numEntriesPerCandidate)
{
  
	uint32_t *localCandidate; 
	uint32_t tmpPv[numLargeEntriesPerQuery][4], tmpMv[numLargeEntriesPerQuery][4];
	uint32_t Ph, Mh, Pv, Mv, Xv, Xh, Eq;
	uint32_t candidate;
	uint32_t idEntry, idCandEntry, idColumn = 0, indexBase, intraBase, mask;
	...
	if((positionRef < sizeRef) && (sizeRef - positionRef) > sizeCandidate){
		...
		for(idEntry = 0; idEntry < numLargeEntriesPerQuery; idEntry++){
			for (i = 0; i < 4; ++i){
				tmpPv[idEntry][i] = MAX_VALUE;
				tmpMv[idEntry][i] = 0;
			}
		}
        ...
        for(idCandEntry= 0; idCandEntry < numEntriesPerCandidate; idCandEntry++){
			...
			for(intraBase = 0; intraBase < BASES_PER_ENTRY; intraBase++){
					...
					for(i = 0, id = 0; id < numEntriesPerQuery; id++, i++){
						idEntry = id / 4;
						i = i % 4;
						Pv = tmpPv[idEntry][i];
						Mv = tmpMv[idEntry][i];
						Eq = h_queries[initEntry + idEntry].bitmap[indexBase][i];
						mask = (id == numEntriesPerQuery-1) ? finalMask : HIGH_MASK_32;
...
int computeAllQueriesCPU(void *reference, void *queries, void *results)
{
	...
	uint32_t numEntriesPerQuery, numLargeEntriesPerQuery, sizeCandidate, sizeQuery, 
			 idCandidate, numEntriesPerCandidate, positionRef, initEntry;
		...
		for (idCandidate = 0; idCandidate < qry->numCandidates; idCandidate++){
			...
			numLargeEntriesPerQuery = (sizeQuery / NUM_BASES_ENTRY) + ((sizeQuery % NUM_BASES_ENTRY) ? 1 : 0);
			numEntriesPerCandidate = (sizeCandidate / BASES_PER_ENTRY) + ((sizeCandidate % BASES_PER_ENTRY) ? 2 : 1);

			if(sizeQuery <= 4096){
				computeMyers(qry->h_queries, ref->h_reference, res->h_results, 
			 				  idCandidate, sizeCandidate, sizeQuery, ref->size, 
							  numEntriesPerQuery, numLargeEntriesPerQuery, positionRef, initEntry, numEntriesPerCandidate);
			}
		}
...
int saveResults(const char *fn, void *results)
{
	...
	#ifdef CUDA
		sprintf(resFileOut, "%s.gemres.pad.gpu", fn);
	#else
		sprintf(resFileOut, "%s.gemres.pad.cpu", fn);
	#endif
